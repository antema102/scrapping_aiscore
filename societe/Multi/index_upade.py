...
def processed_in_tabs(file_path,tab,sheets,driver):

    driver.switch_to.window(tab)

    print(f"Traitement dans l'onglet {tab} pour {sheets}")
...
def societe():
    ...
    """
    Fonction pour lancer les traitements en simultané avec un seul WebDriver mais plusieurs onglets.
    """
    try:
        ...
        files_and_sheets = []  # Liste des fichiers et des feuilles à traiter

        # Exemple de construction dynamique des parts à traiter
        for dep in range(34, 35):  # Départements de 8 à 12
            dep_formatted = str(dep).zfill(2)
        ...
        # Ouvrir les onglets dynamiquement pour chaque feuille
        for file_path, sheets in files_and_sheets:
            for sheet_name in sheets:
                driver.execute_script("window.open('https://www.societe.com/cgi-bin/recherche', '_blank');")
                tabs = driver.window_handles  # Récupérer les onglets ouverts

                # Lancer un thread pour chaque onglet et traiter
                thread = threading.Thread(target=processed_in_tabs,args=(file_path, tabs[-1], sheet_name, driver)) 
                thread.start()
                time.sleep(2)  # Attendre un peu avant d'ouvrir le prochain onglet

        # Attendre que tous les threads se terminent (optionnel)
