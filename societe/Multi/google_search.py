...
def societe(file_path, sheets):
    try:
        ...
        proxies = [
            'http://antema103.gmail.com:9yucvu@gate2.proxyfuel.com:2000',
            'socks5://antema103.gmail.com:9yucvu@gate2.proxyfuel.com:3000'
        ]
            ...
            for i, row in enumerate(worksheet.iter_rows(min_row=1, values_only=True), start=1):
                ...
                try:
                    ...
                    for start in range(0, MAX_RESULTS, STEP):
                        # Effectuer la recherche avec le proxy
                        results = search(query, num_results=STEP,sleep_interval=5, lang='fr', proxy=proxy, ssl_verify=False,timeout=10)
