...
def societe(file_path, sheets):
    try:
        ...
        chrome_options.add_argument("Mozilla/5.0 (Linux; Android 10; Pixel 3 XL Build/QP1A.190711.020) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/91.0.4472.124 Mobile Safari/537.36")
        ...
        try:
            ...
            for i, row in enumerate(worksheet.iter_rows(min_row=1, values_only=True), start=1):
                ...
                query = f'{name_company} {code_postal} {commune} societe.com'
                ...
                try:
                    ...
                    WebDriverWait(driver, 10).until(
                        EC.presence_of_element_located((By.CSS_SELECTOR, 'span[jsname="UWckNb"] a[jsname="UWckNb"]')))

                    urls = driver.find_elements(
                        By.CSS_SELECTOR, 'span[jsname="UWckNb"] a[jsname="UWckNb"]')
                                ...
                                for attemp_societe in range(max_societe):
                                    try:
                                        new_response = requests.get(
                                            href, headers=headers, proxies=proxy, timeout=10, verify=False)
