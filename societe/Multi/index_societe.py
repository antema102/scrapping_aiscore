...
for dep in range(75, 76):  # Départements de 8 à 12
    ...
    parts = [f"part_{j}" for j in range(6, 7)]  # Générer part_1 à part_6
    ...
    try:
        ...
        try:
            ...
            for i, row in enumerate(worksheet.iter_rows(min_row=1, values_only=True), start=1):
                ...
                url = f'https://www.societe.com/cgi-bin/liste?ori=avance&nom={name_company}&exa=on&dirig=&pre=&ape=&dep={number}'
                found_match = False
                # Valeurs par défaut
                ...
                try:
                    lien_societe = url
                    response = requests.get(lien_societe, headers=headers, timeout=5, verify=False)
                    try:
                        if response.status_code == 200:
                            ...
                            elements = soup.select(
                                    'a.ResultBloc__link__content')

                            if not elements:
                                try:
                                    h1=soup.select_one('#appMain > div > section > div > h1')
                                    if h1:
                                        print("ip bloquer")
                                        return False
                                ...
                                try:
                                    for item in elements:
                                        try: 
                                            sirene = item.select_one('p:nth-child(3)').text.strip()

                                            # Nettoyage des espaces
                                            sirene_result = sirene.split(' ')
                                            sirene_number = sirene_result[-1].replace(" ", "")

                                            sirene_test = sirene.split(':')
                                            if len(sirene_test) > 1:
                                                siren___ = sirene_test[1].strip()
                                            else:
                                                siren___ = sirene.strip()

                                            if len(sirene_number) >= 4:
                                                last_four_digits = sirene_number[-4:]
                                            else:
                                                last_four_digits = sirene_number

                                            href = item.get("href")
                                            href_societe = f"http://www.societe.com/{href}"

                                        except Exception as e:
                                            print("error e", e)

                                        if last_four_digits == last_four_digits_sirene:
                                            response = requests.get(href_societe, headers=headers, timeout=5, verify=False)
                                            if response.status_code == 200:
                                                soup = BeautifulSoup(
                                                    response.text, 'html.parser')
                                                ...
                                                if status_entreprise:
                                                    ...
                                                    for item in li:
                                                        try:
                                                            ...
                                                            if span_text == 'ADRESSE':
                                                                span_adresse_tag = item.select_one('span:nth-child(2)')
                                                                if span_adresse_tag:
                                                    ...
                                                    worksheet.cell(row=i, column=1, value=siren___)
                                    ...
                                    print('erreur lors récupération',e)
                                
                    except Exception as e:
                        print("erro lors récuper des liens", e)
                ...
                if not found_match:
                    print(f"Aucun sirene correspondand {name_company}")
                    processed_elements.add(f"{last_four_digits_sirene} {name_company}")
                    save_processed_element(last_four_digits_sirene, name_company, processed_filename)
                else:
                    print(f"Donnée crée {denominations}")
                    processed_elements.add(f"{last_four_digits_sirene} {denominations}")
                    save_processed_element(last_four_digits_sirene, denominations, processed_filename)
